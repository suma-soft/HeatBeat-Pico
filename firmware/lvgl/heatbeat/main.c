...
static int try_wifi_auth(uint32_t auth)
{
    ...
    int rc = cyw43_arch_wifi_connect_timeout_ms(WIFI_SSID, WIFI_PASS, auth, 25000); // Zwiększony timeout
    if (rc) { 
...
static void startup_sync_with_server(void) {
    if (!have_ip_up()) {
        ...
        printf("[STARTUP] Brak połączenia IP - pomijam sync\n");
        return;
    }
    ...
    main_screen_show_status("Łączenie z serwerem...", false);
    ...
    if (st == HB_HTTP_OK) {
        ...
        main_screen_show_status("Połączono z serwerem", false);
        printf("[STARTUP] main_screen_show_status zakończone\n");
        ...
        if (wifi_connected && have_ip_up() && recovery_attempts_count < 3) {
            ...
            if (should_check_server) {
                ...
                if (gst == HB_HTTP_OK) {
                    ...
                    main_screen_show_status("Błąd komunikacji z serwerem", true);
